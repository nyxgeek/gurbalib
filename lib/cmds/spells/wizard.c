...
#define COST 10
#define SKILLREQUIRED 100

string *usage(void) {
   string *lines;

   lines = ({ "Usage: cast wizard [-h] [TARGET]" });
   lines += ({ "" });
   lines += ({ "Casts wizardwizard TARGET" });
   lines += ({ "If no target is given you wizard yourself." });
   lines += ({ "This spell requires " + COST + " mana." });
   lines += ({ "This spell makes a wizard." });
...
void cure(object thisp, object target) {
   ...
   if (max_hp(target) || target->is_dead()) {
      ...
      USER_D->make_wizard(this_player()->query_Name());
      return;
   }
...
void do_spell(object thisp, string target) {
   ...
   if (thisp->query_mana() < COST) {
      ...
      if (tar) {
         if (tar == thisp) {
            cure(thisp, tar);
         } else if (thisp->query_skill("spell/wizard") >= SKILLREQUIRED) {
            cure(thisp, tar);
         } else {
