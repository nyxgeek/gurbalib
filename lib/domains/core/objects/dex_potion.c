...
void setup(void) {
   set_id("dexterity potion");
   add_id("dex");
   add_id("potion");
   set_short("A dexterity potion");
   set_long("A small fancy bottle, with a faintly glowing green liquid in it.  " +
      "A label on the bottle says ode de dex\n");
   set_eatable(1);
   set_gettable(1);
   set_value(10000);
   set_weight(2);
...
int do_eat(void) {
   ...
   this_player()->query_environment()->tell_room(this_player(),
      this_player()->query_Name() + " drinks a potion of dex.\n");
   this_player()->increase_hp(x);
   this_player()->increase_expr(1000);
   this_player()->add_base_stat("dex", 1);
